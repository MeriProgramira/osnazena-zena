...
<p><a class=" m-3" href="{{ route('dashboard') }}"> <b>Povratak na Dashboard</b> </a></p>
    <div class="testbox">
        <form action="{{ route('update-post', $post) }}" method="POST" enctype="multipart/form-data">
            ...
            @method('PUT')
        <div class="banner">
          <h1>"Osnažena žena" - ažuriraj post</h1>
        </div>
        ...
        <div class="item">
            <label for="keywords">Kljucne rijeci<span>*</span></label>
            <input id="keywords" type="text" name="keywords" value="{{ $post->keywords }}" />
            @error('keywords')
              <div class="alert text-danger py-1">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="btn-block">
          <button type="submit" >Sačuvaj post</button>
        </div>
      </form>
    </div>
